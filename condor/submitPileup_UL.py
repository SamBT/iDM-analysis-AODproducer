...
def buildSubmit(infile, workpath, mode, uid, year):
    ...
    stageOutPiece = '''
remoteDIR="/store/group/lpcmetx/iDM/Samples"
for f in `ls *AOD*.root`; do
    ...
    try:
        if mode=='lhe':
            ...
            os.system('cp runOffGridpack%sPileupCondor_UL.sh %s/submit' % (year,workpath))
            os.system('cp DIGIRAW_template_%s_UL.py %s/submit' % (year,workpath))
            with open('%s/submit/runOffGridpack%sPileupCondor_UL.sh' % (workpath,year), 'a') as f:
                f.write(stageOutPiece)
    except:
...
exit 0'''
    with open(workpath + '/exec.sh', 'w') as f:
        ...
        if mode == 'lhe':
            ...
            f.write(execF % (('runOffGridpack%sPileupCondor_UL' % year), infile+' '+ctau))
